...
Avant de nous lancer tête baissée dans de la matière nouvelle, assurez vous d'avoir installé la SciViews Box `r learndown$svbox`. Si ce n'est pas encore fait retournez à la [page d'acceuil pour bien débuter]( `r learndown$baseurl` )

Consacrez un petit quart d'heure à repérer les nouveaux icônes dans le Dock et dans le menu `Applications`. Vous retrouverez R et RStudio, mais dans des version plus récentes qui apportent également leur lot de nouveautés. 
...
Nous pouvons nous demander si notre modèle 2 qui est la meilleure droite de nos quatre modèles est le **meilleur modèle possible dans l'absolu**. Pour se faire nous allons devoir définir une technique d'optimisation qui nous permet de déterminer quelle est la droite qui minimise notre fonction objective. Dans la suite, nous garderons le critère des moindres carrés (des résidus).
...
assignation("sdd2_module01", part = NULL,
  url = "https://github.com/BioDataScience-Course/sdd2_module01",
  course.urls = c(
    'S-BIOG-015' = "https://classroom.github.com/a/CNxriMps"),
  toc = "Assignation : Exercices pratiques sur la régression linéaire simple.")
```

## Récapitulatif des exercices

Ce module 1 vous a permis de prendre connaissance des principes de bases de la modélisation et plus principalement de la régression linéaire. Pour évaluer votre compréhension de cette matière vous aviez les exercices suivants à réaliser\ :
