...
- `dissimilarity(data, method = "euclidean", scale = FALSE, transpose = FALSE)` \ : crée une matrice de dissimilarité. L'argument `method =` prends les valeurs `"euclidean"`, `"manhattan"`, `"bray"`, `"canberra"`, ... et bien d'autres. Voyez l'aide de `?vegan:vegdist` et `stats:dist` pour les détails. Un objet "dissimilarity" est créé et les méthodes `print()`, `chart()`, `labels()` et `nobs()` sont disponibles,
- `cluster(x, method = "complete")`\ : part d'un objet "dissimlarity" dans `x` et calcule  le regroupement  à l'aide de différentes méthodes (`"complete"`, `"single"`, `"average"`, `"ward.D2"`, ...). Voir l'aide en ligne de `?hclust` pour la liste complète et les explications. Un objet "cluster" est créé, sur lequel nous pouvons appliquer les méthodes `print()`, `str()`, `plot()`, `chart()`, `labels()`, `nobs()`, `predict()` et `augment()`.
- Le dendrogramme (graphique qui visualise la CAH et qui sera expliqué plus loin) est créé à partir d'un objet "cluster". Un dendrogramme vertical est créé paer défaut avec `chart(x)` ou `chart$vertical(x)` pour être plus explicite. Une version horizontale s'obtient avec `chart$horizontal(x)`, et une version circulaire avec `chart$circular()`.
- Le niveau de coupure dans le dendrograme est indiquant en ajoutant `geom_dendroline()` au graphique (`chart(x) + geom_dendroline(h = 5)`.
- Les groupes sont extraits avec `predict(x, h = 5)` ou `predict(x, k = 5)`. `h =` est le niveau de coupure et `k =` est le nombre de groupe souhaité.
- `augment(data = df, x, h = )` extrait aussi les groupes mais les rajoute comme nouvelle colonne `.fitted` dans le tableau `df`. On peut aussi utiliser `k =` à la place de `h =` ici.
...
zoo %>.%
  ...
  chart$horizontal(zoo_clust) + # Dendrogramme horizontal (plus lisible)
    geom_dendroline(h = 70, color = "red") + # Séparation en 3 groupes
    theme(axis.text.y = element_blank()) +   # N'affiche pas les labels
    ylab("Hauteur")
```
